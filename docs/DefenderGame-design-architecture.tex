...
\fancyhead[R]{\small\color{Muted}源码基线 1.7.0}
\fancyfoot[L]{\footnotesize\color{Muted}DefenderGame · 2026-09-22}
...
源码版本 & \code{1.7.0-windows}；规则 config v12；存档 schema v6\\
分支 / 提交 & \code{windows-godot} / \code{dd32a33} + 本轮工作树\\
分析日期 & 2026 年 9 月 22 日\\
交付内容 & 可独立编译的 LaTeX 源文件、PDF 设计与架构文档\\
证据口径 & 当前代码与配置优先；历史文档用于追溯设计意图\\
\end{tabularx}
\vfill
\note{本文是源码 1.7 的工程基线，不是原版 Defender II 的精确规格。本轮在既有素材与规则基础上优化界面、特效和音频，修复三项玩法缺陷，并同步需求、架构与验证记录。未改动真实玩家存档，未提交、推送或重新导出发行包。实装机制、测试证据与剩余风险分别标注。}
\vspace{8mm}
...
基线提交完整标识为 \code{dd32a331fc955d5f9b4ff5898122ea8d8c5e1dfc}。开始分析时工作区干净。当前统计：41 个生产 GDScript、3 个场景、25 个顶层测试 GDScript、21 张游戏 PNG；\code{GameArt} 登记其中 20 张，旧 \code{主城墙.png} 未用于当前城墙绘制。测试支持目录另有失败保存替身。分析不读取真实 \code{savedata/} 的玩家数据，不重新验证外部网站与原作数值。

\subsection{总体判断}
工程已经具备完整的“装备与研究、进入关卡、固定步长战斗、持久化结算、下一关”离线闭环。主要优点是规则可脱离场景运行、共享属性目录减少预览与实战漂移、战斗 RNG 分流、保存优先事务、内容校验和较广的回归脚本。

主要维护压力集中在 \code{main_menu.gd}、\code{gameplay.gd} 与 \code{run_model.gd} 三个大文件。当前分层是实用的逻辑分离，不是严格端口适配器架构：应用层直接使用 Godot 文件 API，表现层依赖具体 \code{GameApp} 与规则查询，战斗子系统通过 model 门面调用内部方法。
...
本轮修订 & 元素荣誉、入场满蓝、败局悬赏已修复；主界面、HUD、法术冲击、分层配乐与暂停优化。剩余边界见第 \ref{sec:risks} 节。\\
\bottomrule
\end{tabularx}

\subsection{证据等级}
“实现”表示当前源码可定位；“本次验证”表示 2026-09-22 实际执行；“设计目标”来自需求，不等同验收结果；“建议”不代表已经实施。原版截图只支持布局或机制研究，项目当前的伤害、价格、控制和增长公式均按本地配置解释。
...
\node[box,below=of save,text width=127mm] (next) {胜利：下一关；失败：重试；均可研究整备；保存失败：重试保存};
...
\item 1.7 首页以金色继续按钮为主入口，六个次级入口为两列网格；显示当前关敌数、波数、出怪窗口与下一次Boss关。窗口失焦自动暂停并取消拖拽，必须手动恢复。
\item 非16:9窗口采用固定画布居中留边，HUD不延伸到规则战场之外。1/2/3与Esc同时接受物理键码和逻辑键码，兼容不提供扫描码的输入源。
\end{itemize}

\subsection{非目标与质量目标}
当前不包含联机 Battle、账号、广告、支付、服务器、移动端、在线排行榜或强制网络。Windows 10/11 x86\_64 是文档目标平台；采用 Compatibility 渲染，逻辑画布 1920×1080、默认显示 1280×720（启动设置可覆盖），\code{canvas_items + keep} 适配。
...
Result & run/stage 身份、胜负、击杀、累计资源、墙百分比、武器、Boss数及 fire/ice/lightning\_casts；应用层据此结算元素荣誉。\\
\bottomrule
\end{tabularx}
...
\note{1.7已补齐Result中的三元素施法数，并用真实施法→结果→保存→重载→重复结算验证荣誉链路。失败结算不提交统计，重试不重计。败局只保留真实击杀收入，不加通关悬赏；胜利仍领取通关悬赏。旧版本未记录的施法次数无法准确还原，不伪造历史补偿。great\_mage的魔力上限加成完成后再初始化mana，满蓝进入关卡。}
...
\subsection{1.7 视觉统一与信息层级}
主菜单、魔法研究、技能按钮和光标共用 \code{MagicCursor.draw_symbol}，\code{ElementIcon}仅负责缩放和元素颜色。复用既有PNG，不新增外部图标字体或图片依赖。暖金用于主操作，青色用于键盘焦点，深蓝底板隔开文字与高对比背景；菜单装饰微粒最多20个，低画质关闭。

战斗顶部硬边暗带改为连续透明渐变，左下HP/Mana、武器、设施使用独立底板；装饰控件鼠标穿透，技能与暂停按钮仍响应输入。Boss血条与反馈文字分区，反馈带优先级，普通命中不抢占Boss警告。新增波次、残敌清理、危墙一次性提示、场上敌数/用时及冷却整数秒；危墙同时用文字与颜色表达。

每次真实落击增加受预算限制的冲击碎片与短闪光。火系使用向上火焰尖舌、热核和烟尘，冰系在地面形成短暂晶刺并保留冰牢，雷系根据落点区分折线形状。飘字增加描边并限制屏内位置。这些变化不新增命中，不改变落点、RNG、半径、3秒窗口或56/52/60发数量。密集场面仍优先保证怪物和HUD可读。

结果面板补用时、施法数、失败建议和研究整备入口；应用使用一次性的 \code{menu_destination} 返回研究页，不将导航状态写入存档。多项荣誉文本自动折行。画面质量预算与真实伤害解耦，低画质仍完整结算所有落击。
...
\subsection{设置与诊断}
简中/英文文本由key查询，缺少语言回退简中，缺key回显key；内容校验保证要求的键存在。窗口、无边框、全屏、音量、自动射击、震动、画质和UI缩放在应用层处理。暂停快速设置新增独立音乐滑杆；保存失败仍回滚旧设置。

\subsection{1.7 分层程序音频}
\code{SoundBank}生成22050Hz、16位单声道PCM：菜单12.8秒、战斗10秒、Boss8秒，均为四小节Am/F/C/G结构，包含铺底、拨弦和低音，战斗/Boss增加节奏层。三段PCM总量小于1.5MB，一次生成缓存，无在线下载或新增音乐素材授权依赖。数学相位和噪声不消耗规则RNG。

\code{DefenderProceduralAudio}用两个音乐播放器在0.8秒内交叉淡化；暂停/结算将背景音乐压至32\%，恢复后平滑回升。音乐零值增益为-80dB；headless不创建播放声部。音效共10声部，8个普通轮换、2个告警/结果保留，射击、命中、元素落击按各自毫秒间隔限流；不重复播放hit与damage两份同一命中。PCM首尾加短包络以减轻接缝；声音素材并非录制配器，主观音色仍需玩家试听。
...
2026-09-22在上述源码基线执行以下验收。表中数量为各脚本自身断言口径，不将不同脚本重叠断言当成独立需求覆盖率。
...
\code{inputmap_acceptance.gd} & headless & 17/17\\
...
\code{presentation_polish_acceptance.gd} & headless / 图形 & 41 / 44项\\
\code{performance_stress.gd} & headless & p95 1.430ms\\
\code{long_soak.gd} & headless，60模拟分钟 & p95 1.465ms\\
\bottomrule
\end{tabularx}
内容校验确认 config12有效；无尽研究脚本包含正常模拟第1000关（研究75）与第10000关（研究200），不是人工强制清怪。固定seed自动瞄准胜利不代表人手操作难度已平衡。

\subsection{性能解释}
压力fixture为100敌人与200箭矢，推进9000tick即5模拟分钟，本机平均1.257ms、p95 1.430ms、墙钟11.31秒，低于10ms门槛。该fixture大部分是无命中静态箭，不能替代最坏穿透、密集落击和真实GPU帧率。三系III合计168发的图形技能专项另测计划约1.60ms、逻辑p95约1.22ms；这些均是本机快照，不是最低硬件承诺。

性能控制包括生成关活怪96上限、箭寿命与最多五箭齐射、空间带碰撞、共享网格缓存、死亡残影48上限。低/中/高画质效果队列12/32/64、飘字8/18/32；这些预算只影响显示，不能丢失核心伤害。

\subsection{额外验证与未重跑项}
60模拟分钟浸泡实际墙钟137.60秒，内存由28.42MiB至31.58MiB，增长3.17MiB、峰值34.23MiB，节点3→3；这是加速纯逻辑测试，不是60分钟实时时长的GPU音频长稳。窗口模式及非16:9映射专项10项通过，图形结果见末节。未重跑杀进程保存恢复、冷启动/工作集测试、全套导出或正式构建；历史数字不作为本轮发布结论。三项已知玩法缺陷已有永久回归，剩余风险仍单列。

\nextsection{发布构建、资源合规与可追溯性}
\subsection{构建链路}
\code{tools/build_windows.ps1}要求干净工作区，顺序执行：内容校验→核心/无尽关卡/无尽研究/技能交互/视听体验/自动通关/存档槽测试→引擎版权声明一致性→PCK预检→项目许可完整性→构建清单→匹配的Godot4.7.2模板→EXE/PCK导出→实际包验收→Release ZIP。
...
\toprule
版本来源 & 当前值\\\midrule
\code{project.godot} & 1.7.0-windows\\
\code{SaveService.APP_VERSION} & 1.7.0-windows\\
主/回退配置 & config12；presentation-polish-v1（保留独立随机倾泻）\\
\code{export_presets.cfg}文件/产品版本 & 1.7.0.0（本轮同步）\\
构建ZIP文件名 & Aegis-of-Ember-1.7.0-Windows-x64.zip\\
\bottomrule
\end{tabularx}
...
\label{sec:risks}
本节区分本轮已修复项和仍保留的边界风险。优先级P1表示影响主要功能/数据可靠性；P2表示一致性、可维护性或边界风险。验证均用隔离测试档案。

\subsection{1.7 已修复并纳入永久回归}
\begin{enumerate}
\item \textbf{元素荣誉统计。}Result加入fire/ice/lightning\_casts，实际三系施法可进入HonorService；测试跨越火系200次门槛、保存失败回滚、重载与重复结算。
\item \textbf{入场魔力。}setup在great\_mage上限计算后赋满mana；荣誉3级无其他研究时为138/138，而不是120/138。
\item \textbf{败局额外悬赏。}clear\_coins的coin\_bounty只在胜利加算；零击杀败局为0，有击杀败局保留真实收入，胜利收益保持。
\end{enumerate}
...
\item \textbf{P2：发布可追溯性。}本轮已同步源码/存档/PE/ZIP版本与主要文档路径，但多个版本常量仍需人工维护，正式1.7包尚未导出。旧版历史验收保留原日期，不代表当前工作树已发布。
\end{itemize}

\subsection{建议实施顺序与验收}
本轮已完成三项功能修复及版本/文档入口同步。后续优先补坏备份故障fixture与恢复；再补回放上下文；随后按职责拆菜单/战场控制器，稳定Command/Snapshot/Result结构，做长期经济与存档规模测试。拆分不改变tick顺序、RNG消耗和奖励键；不同时重写所有架构。
...
\small
下表覆盖当前41个生产脚本，目录简写均相对仓库根。修改定位以路径和函数名为准，行号会随后续开发变化。
...
\path{src/presentation/art/element_icon.gd} & 复用魔法光标形状的可缩放元素图标。\\
...
\path{src/infrastructure/audio/sound_bank.gd} & 三段四小节配乐与12类短音效PCM生成。\\
\bottomrule
\end{longtable}
\normalsize
...
\subsection{本地依据清单}
\begin{enumerate}
\item \path{project.godot}、\path{scenes/}、\path{src/}：版本、真实场景和41个生产模块。
...
\label{sec:finalqa}
真实Compatibility图形验收：双语×三分辨率菜单/战斗布局0失败；素材75项、城墙54项、技能交互369项、体验44项、窗口10项通过，包含可选截图检查。像素样本避开新增HUD，布局测试不再依赖结算页最后一个子节点；最终进程检查退出码与标准错误，不能将首轮失败日志误算为通过。

计算机技能实际操作隔离试玩：下拉换装、战斗施法、胜败结算、研究整备入口、暂停与音乐滑杆保存均已观察。音频验证覆盖PCM峰值/边界、声部/限流/增益，未作主观试听评价。日志和游戏截图位于忽略目录 \path{Builds/polish-review-20260922/}，其他专项按各自目录保存。

PDF用XeLaTeX编译并渲染全部页面检查缺字、引用、边界与分页。保留源文件与PDF；TeX中间文件仅在忽略的Builds临时目录，清理操作被环境策略阻止，尚未删除。测试证明所列fixture和本机行为，不等同最低硬件承诺、全部高关卡人工平衡或完整发布认证。真实玩家存档与原PNG未修改。
