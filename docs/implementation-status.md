...
版本：1.7.0-windows（以 `project.godot` 的 `application/config/version` 为准）

文档日期：2026-09-23

分支：`windows-godot`

> 文档口径：§1～11 保留历史原始验收证据。当前 1.7 基线见 §12，后续技能及城防表现增量见 §13～14；旧发行包不会自动包含未重新导出的源码改动。
...
| 项目 | 当前结果 | 依据 |
|---|---|---|
| 应用/规则版本 | `1.6.0-windows` / config v11 / `aegis-windows-independent-barrage-v1` | `project.godot`、`content/config/game_rules.json` |
...
| 技能交互专项 | **342 passed / 0 failed** | `tests/skill_interaction_acceptance.gd`（本轮运行） |
...
当前发布候选为 `Aegis-of-Ember-1.6.0-Windows-x64.zip`；已发布的 v1.0.3～v1.5.0 不包含独立随机落点与紧凑技能范围，不应继续分发。
...
## 11. 独立随机落点与紧凑技能范围（2026-09-09）

config v11 / `aegis-windows-independent-barrage-v1`，主规则与回退包同步。按最新要求废止上一节 v10 的人为空区与覆盖率门槛，删除中部避让圆、抖动格和最远点筛选；三阶每一发分别均匀抽取战场内 x/y，允许自然相近或漏击，不保证任何覆盖百分比。范围升级不再改变同种子的落点。保留 3 秒窗口、56/52/60 发、原下落速度与随机发射间隔，一/二阶指定点与指定区域规则不变。

九技能基础直击/溅射半径缩小，研究每级半径增长从 +4/+6 改为 +1/+1，通用范围研究从每级 +6% 改为 +3%（上限 5 级）。研究 20 级 + 通用范围 5 级时，三阶火/冰/雷的直击/溅射半径约为 119.6/188.6、125.3/200.1、119.6/194.3 逻辑像素。每颗伤害、状态参数、冷却、蓝耗和无尽伤害成长不变；旧档保留研究等级，属性预览与实战统一采用新参数。

本轮技能交互专项 **342/342 无头、369/369 真实渲染**：108 种子/等级组合逐发对照独立均匀随机序列，确认中部可落击、升级不移动落点；增加九技能范围增幅/通用加成/封顶回归，删除所有覆盖率区间断言。100 敌人三系同放 168 发全部结算：规划约 **1.66 ms**，模拟 tick p95 **1.37 ms**（本机真实渲染模式测量）。

核心 **162/162**、技能链 **309/309**（无头及真实渲染）、无尽研究 **286/286** 通过；研究等级 75/200 的第 1000/10000 关及原 30 关 + 9 个生成关自动战斗全部胜利；双语 × 三分辨率真实渲染布局 0 失败。修正核心范围研究测试的旧 200 像素硬编码探针，改为在当前基础/升级后溅射半径之间验证命中。内容校验和差异空白检查通过，无脚本错误。

运行日志为 `Builds/skill-interaction-review/*-independent*.log`，截图在该目录及 `Builds/skill-chains-review/`（均不入库）。详细参数与机制见 [三级技能链说明](skill-chains-reimplementation.md)。

## 12. 视听与离线体验优化（2026-09-22，源码 1.7.0）

基线 `dd32a33`，源码/存档/PE/ZIP 版本统一 1.7，主/回退 config12，schema6 不变。完整设计、理由、验收限制见 [视听与体验优化](presentation-polish.md)，总设计更新到 `DefenderGame-design-architecture.tex` 和同名 PDF。

- 首页主次入口分层、战前摘要与 Boss 预警；元素图标跨菜单/研究/战场/光标统一。
- HUD 状态底板、冷却秒数、Boss/告警分区和反馈优先级；补波次/残敌/危墙提示、描边飘字。
- 保留已有图片、内院遮挡和完整普通/岩浆背景；强化火焰/冰晶/雷击冲击，改连续背景渐变。固定战场等比居中，非 16:9 不出现扩展的无效战区。
- 三套分层程序音乐、双声部 0.8 秒淡化、暂停/结果降音量、十声部音效限流与告警保留；暂停增加音乐滑杆。
- 失焦自动暂停、结果直达研究；物理/逻辑快捷键兼容。修复真实施法到元素荣誉的断链、荣誉魔力入场未满、零击杀败局发通关悬赏。
- 不改变无尽关卡/研究、武器和技能随机/范围曲线，不增加联机、外部依赖或新生成图片。

最终验收：核心 **162/162**，技能链 **309/309**，攻击 **196/196**，无尽关卡 **166/166**，无尽研究 **286/286**，事务 **4/4**，存档槽 **18/18**，武器 **30/30**，菜单 **12/12**，设置 **8/8**，输入 **17/17**，诊断 **10/10**，教学 **8/8**。新体验专项 headless **41/41** / 图形 **44/44**，技能交互 **342/342** / **369/369**，素材图形 **75/75**，城墙图形 **54/54**。中英文三分辨率布局零失败，窗口模式及非 16:9 逻辑坐标 **10/10**。

自动通关原 30 关 + 9 个生成关全胜；高研究专项正常通关第 1000/10000 关。100 敌人/200 箭逻辑压力 p95 **1.430 ms**；60 模拟分钟浸泡 p95 **1.465 ms**，内存 **28.42→31.58 MiB**、峰值 **34.23 MiB**，节点 **3→3**。本机图形样例平均帧间隔 **8.18 ms**。这些不是最低硬件认证或真实一小时图形/音频长稳。

计算机技能实际操作隔离档：下拉换装、火系真实施放、胜败结算、研究整备入口、暂停及音乐滑杆保存。键盘自动输入缺物理扫描码的问题新增逻辑键码支持；最终 InputMap 回归同时测两种事件。修正了旧测试对结算页末节点、被 HUD 遮挡的城墙像素、headless 鼠标原点的依赖；保留原功能和像素阈值，并重新验证标准错误。

全部测试串行隔离真实存档；输出在 `Builds/polish-review-20260922/` 及专项原目录。未主观评定音乐音色，未重跑杀进程保存恢复、冷启动/进程工作集或正式导出。正式包必须待代码审阅提交后从干净工作树生成；本轮不自动提交或 push。

## 13. 三系技能视听材质重制（2026-09-22，源码工作树）

本节是 §12 之后的增量，保持 1.7/config12/schema6，规则与存档格式未变。新增三张 imagegen 透明图集和 `SpellVisuals`，火球拖尾/爆燃/余烬、冰锥/晶簇/冰牢、分叉雷击/接地电光/眩晕电弧取代技能几何占位。六段原创合成音效区分发射和命中，十声部按 4+2+2+2 隔离；血条在命中层之上。既有图片和技能随机/范围/伤害参数不改。

专项 headless **87/87**、图形 **91/91**；核心 **162/162**、技能链 **309/309**、技能交互 **342/342** / **369/369**、素材 **75/75**、体验 **41/41** / **44/44**、输入 **17/17** 全通过，图形日志无脚本错误。100 怪物三系同时倾泻图形样例平均 **15.29 ms/帧**。计算机技能在隔离实际关卡中按 1/2/3 和鼠标分别施放验证。PCM 端点、峰值与路由已测，未声卡回录或主观试听评分。

完整设计/边界见 [三系视听重制](elemental-vfx.md)，提示词与生成模式见 [生成记录](elemental-vfx-prompts.md)。更新 Markdown 需求、架构、素材、技能及开发说明；本次表现层增量尚未重新编入上一轮总设计 PDF。源码与素材未提交/push，未重新导出 EXE/PCK，不绕过干净工作树构建门禁。

## 14. 九技能图标与城防视听强化（2026-09-23，源码工作树）

续作完成 2026-09-22 中断的图标与城防强化，版本仍为 1.7/config12/schema6。新增九技能 3×3 图集、城防 2×2 部件和两张配对横屏地面，原文件保留。图标在菜单/下拉/研究/HUD/光标同源分阶；城防增加弩机回弹装填、水晶塔放电、沟内熔岩泡沫/火星、墙面碎屑与石材压边。四种原创城防音效加熔岩循环环境音，使用独立防御/环境声部，不抢既有技能与告警声部。

`fortress_art_acceptance.gd` 已进入正式构建门禁，新图集/地面/渲染和音效脚本加入 PCK 必需路径。续作补充弩机、水晶与熔岩动画像素差，暂停后像素不变，以及恢复/普通地面/退出时环境音生命周期验证。保持发射原点、城墙/内院遮挡、战斗规则和玩家存档不变。

| 2026-09-23 验证 | 结果 |
|---|---|
| 城防专项 | headless **101/101**；图形 **115/115** |
| 核心 / 技能链 / 技能交互 | **162/162**、**309/309**、headless **342/342** / 图形 **369/369** |
| 三系特效 / 体验 | headless **87/87** / 图形 **91/91**；headless **41/41** / 图形 **44/44** |
| 素材 / 城墙内院 | headless **79/79** / 图形 **85/85**；headless **34/34** / 图形 **54/54** |
| 输入 / 武器 / 设置 / 菜单 | **17/17**、**30/30**、**8/8**、**12/12** |
| 分辨率 / 窗口 | 双语×三分辨率 headless/图形均零失败；窗口及非 16:9 **10/10** |
| 自动通关 / 核心压力 | 30 编排关 + 9 生成关全部胜利；100 敌人/200 箭 p95 **1.737 ms** |
| 图形压力样例 | 100 怪物/200 箭平均 **16.45 ms/帧**；三系倾泻 **16.32 ms/帧**，含帧同步，非最低硬件认证 |
| 诊断 PCK | headless 城防 **102/102** / 素材 **80/80**；仓库外图形城防 **116/116**，验证导入映射与测试/参考资源排除 |

计算机技能实际打开隔离档，查看三阶下拉，进入第 30 关，使用 1/2/3 和鼠标施放三系，检查分阶魔法光标、冷却/蓝耗、城防交战及暂停，随后关闭窗口。音效完成 PCM 和路由技术测试，未做主观音色评分或声卡回录。日志和截图见 `Builds/fortress-art-review/`，各旧专项截图仍在原目录；运行日志无脚本错误。

详细设计见 [城防视听强化](fortress-art.md)，完整工具记录提示词见 [生成记录](fortress-art-prompts.md)。需求/Markdown 架构/开发与素材文档已同步，总设计 PDF 未重编。仅生成忽略目录下的诊断 PCK，不是正式发行或清单通过证明；未提交、push、生成新 EXE 或覆盖旧 ZIP，正式发行仍待代码审阅提交后从干净工作树构建。

## 15. 城墙、弩机与整机 UI 重制（2026-09-23，源码工作树）

根据原素材双塔连墙、中央安装台、右向射击的构图，重新生成完整石材城墙和独立机械弩机，保留真实透明通道；新测量安装环、旋转足、水晶插槽及受击边缘，取消重复弩座。保留核心发射点、内院不透明覆盖及战斗规则，旧原图不覆盖。版本仍为 1.7/config12/schema6。

新增绘制九宫格皮肤与十六个统一图标，重做首页导航、顶栏、研究、荣誉、存档、设置、教学、暂停及结算的材质与排版。战斗采用独立金属外壳/珐琅内槽的血量、法力、Boss 与关卡进度条，状态区移出城墙包络；装备、城防等级、冷却与蓝耗分区。研究节点名称与等级/价格分两行，不压边框；英文长技能详情与全部九技能在低分辨率下检查。管理员功能保留但默认折叠。

| 本轮验证 | 结果 |
|---|---|
| 新整机 UI 专项 | headless **908/908**；最终图形 **1036/1036**，64 张截图 |
| 布局 | 双语 × 1280/1366/1920/2560 宽度 headless 零失败，含百万级关卡/研究 |
| 窗口模式 | 图形 **15/15**；修复非窗口设置重置最大化状态，新增五种设置回归 |
| 城墙 / 城防 / 素材 | 图形 **54/54**、**114/114**、**91/91** |
| 技能交互 | headless **342/342** / 图形 **369/369** |
| 核心 / 体验 / 输入 | **162/162**、**41/41**、**17/17** |
| 菜单 / 武器 / 设置 / 存档 | **12/12**、**30/30**、**8/8**、**18/18** |

真实鼠标键盘打开隔离第 30 关，检查菜单下拉、设置滑杆、技能选择/施放、蓝耗冷却与暂停，窗口已关闭。最终 Godot 编辑器导入成功，无脚本错误。整机截图是专项注入场景的实际渲染，不是连续战斗录像。城防比 §14 少一项，是旧弩身 UV 裁切已被完整弩机源图替代，动画/暂停像素门槛未降低。

源码、四张新增素材、提示词和测试已接入；正式构建门禁新增整机 UI / 四分辨率检查，PCK 必需资源表同步。说明和生成来源见 [整机 UI 重制](production-ui.md) 与 [完整提示词](production-ui-prompts.md)，日志和截图位于 `Builds/production-ui-review/`。本轮不更新规则、总设计 PDF，不自动提交/push，未生成新正式 EXE/PCK/ZIP；§14 的诊断 PCK 不包含本节 UI。最低硬件、发行签名及长时人工验收仍属后续发行范围。
