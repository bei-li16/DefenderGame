...
- **全屏倾泻规划（config v11）**：删除候选格、最远点筛选与避让空区；`DefenderSkillSystem.landing_point` 在 screen 模式下通过独立 `_spell_rng` 为每一发均匀抽取战场内 x/y 坐标，不使用范围或覆盖率重新抽样。发射时间继续在 90 tick 窗口不放回抽样；下落速度、数量、伤害衰减和状态逻辑不变。缩小单发基础半径及其研究增量，`SkillCatalog` 继续统一提供研究预览、战斗和视觉快照；旧档等级不变，研究 20 级后仅伤害继续成长。详见技能链说明的随机分布与范围验收。
